...
//            $select .= " and month(dgmobi_buynow_created_on) >= 11"; 
	      $select .= " and month(dgmobi_buynow_created_on) >= 11 and dgmobi_buynow_email not like '%ideabytes.com'";
...
//            $stmt->debugDumpParams();die;
     ...
     * @return array Associative array of transaction details
     */
    public function getTransactionInfoByEmail($email)
    {
        try {
            $select = "SELECT * FROM `dgmobi_buynow_registration_info` "
                    . "WHERE `dgmobi_buynow_email` = :email LIMIT 1";
            ...
            Logger::writeMessage([
                "Type" => "ERROR",
                "Class" => "Transaction",
                "Method" => "getTransactionInfoByEmail",
                "Description" => "Error fetching transaction info based on Email Id",
                "Exception" => "Code: " . $ex->getCode() . "; Message: " . $ex->getMessage(),
                "Data" => ["email" => $email]
            ]);
        }
