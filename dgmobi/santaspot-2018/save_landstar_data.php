...
//    $taxPrice = $data["taxPrice"];
    ...
    $trndetails = $transactionHandler->getTransactionInfoByEmail($email);
    $email_exits = $trndetails['dgmobi_buynow_email'];
    
    if(!empty($trndetails))
//    if(count($trndetails)>0)
        die("registered");
    
    
    $inserted = $transactionHandler->storeDetails(
            $orderId, $totalNoOfLicenses, $totalPriceWithTax, $taxRate, $taxPrice, $phone, $firstName, $lastName, $email, $countryName, $provinceName, $address, $selectedLanguage
    );
    ...
    if ($inserted["rowCount"]) {
        ...
        foreach ($data["products"] as $product) {
            $transactionHandler->storeCartItems(
                    $inserted["lastInsertId"], $product["id"], $product["noOfLicenses"], $product["totalPrice"]
            );
        }
